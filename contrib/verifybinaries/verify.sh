...
###   proofofgamingcore.org and proofofgaming.org and compares them.
...
WORKINGDIR="/tmp/proofofgaming_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://proofofgaming.org"
HOST2="https://proofofgaming.org"
BASEDIR="/bin/"
VERSIONPREFIX="proofofgaming-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'proofofgaming-',
...
if [ $? -ne 0 ]; then
   echo "proofofgaming.org failed to provide signature file, but proofofgamingcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "proofofgaming.org and proofofgamingcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the ProofOfGaming Core binary release signing key installed?"
   fi
