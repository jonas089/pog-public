...
The RPM spec file provided here is for ProofOfGaming-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://proofofgaming.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[ProofOfGaming git repository](https://github.com/ProofOfGamingProject/ProofOfGamingcoin) but are not part of
...
Currently only `Source10` is used, for the example `proofofgaming.conf` file.
...
in the proofofgaming git repository. At present only an SVG version of the ProofOfGaming
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the proofofgaming
...
needed when 0.12.1 is released, a proper fix is already in the ProofOfGaming git
master branch.
...
idea to build ProofOfGaming Core against a static version of that library compiled
...
The `proofofgaming.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `proofofgaming-server` package. At this point, most RPM based
...
The `proofofgaming-server` package should have SELinux support. How to properly do
...
ProofOfGaming-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/proofofgaming/). He has been
packaging ProofOfGaming for Fedora at least since 2012.
...
4. I renamed the `proofofgaming` package that contains the Qt GUI to `proofofgaming-core` as
that appears to be how the general population refers to it, in contrast to
`proofofgaming-xt` or `proofofgaming-classic`. I wanted to make sure the general population
